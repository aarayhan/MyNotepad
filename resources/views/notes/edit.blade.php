    ...
    <div class="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm shadow-zinc-200/40">
        <form method="POST" action="{{ route('notes.update', $note) }}" class="space-y-5">
            ...
            <div class="flex flex-col gap-3 sm:flex-row">
                <button type="submit" class="rounded-lg bg-zinc-900 px-4 py-2 text-sm font-medium text-white hover:bg-zinc-800">Save changes</button>
                <a href="{{ route('notes.show', $note) }}" class="rounded-lg border border-zinc-300 px-4 py-2 text-center text-sm font-medium text-zinc-700 hover:bg-zinc-100">Cancel</a>
            </div>
        </form>
    </div>
</x-layouts.app>
